...
def extract_java_calls(file_path: Path, extraction_result: dict) -> list[_CallSite]:
    ...
    for edge in edges:
        ...
        if callee_name.startswith("new ") or "<init>" in callee_name:
            is_constructor = True
            callee_name = callee_name.replace("new ", "").replace("<init>", "").strip()
        ...
        if "#" in callee_name:
            ...
            receiver = parts[0] if not receiver else receiver
            callee_name = parts[1]
