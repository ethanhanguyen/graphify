...
def extract_go_calls(file_path: Path, extraction_result: dict) -> list[_CallSite]:
    ...
    for edge in edges:
        ...
        if callee_name.startswith("go ") or callee_name.startswith("defer "):
            callee_name = callee_name.split(" ", 1)[-1] if " " in callee_name else callee_name
