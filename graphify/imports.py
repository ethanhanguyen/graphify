...
def _resolve_wildcard_leaf(target_name: str, from_file: Path, all_files: dict[str, list[str]]) -> Optional[str]:
...
def _resolve_wildcard_transitive(target_name: str, from_file: Path, all_files: dict[str, list[str]]) -> Optional[str]:
    candidates = []
    for file_list in all_files.values():
        for f in file_list:
            f_stem = Path(f).stem
            if f_stem == target_name:
                candidates.append(f)
    if not candidates:
        return None
    if len(candidates) == 1:
...
def _resolve_namespace(target_name: str, from_file: Path, all_files: dict[str, list[str]]) -> Optional[str]:
...
def resolve_import(target_name: str, from_file: Path, all_files: dict[str, list[str]], language: str) -> Optional[str]:
    result = _resolve_named(target_name, from_file, all_files)
    if result:
        return result

    result = _resolve_wildcard_leaf(target_name, from_file, all_files)
    if result:
        return result

    result = _resolve_wildcard_transitive(target_name, from_file, all_files)
    if result:
        return result

    provider = get_provider(language)
    if provider and provider.import_semantics() == "namespace":
        return _resolve_namespace(target_name, from_file, all_files)
...
def resolve_all_imports(
    ...
    for imp in imports:
        ...
        if not import_name:
            continue
        result = resolve_import(import_name, file_path, all_files, language)
        if result:
